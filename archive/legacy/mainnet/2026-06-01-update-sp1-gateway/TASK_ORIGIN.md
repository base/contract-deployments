...
stored in `mainnet/signatures/2026-06-01-update-sp1-gateway/`.

Run these commands only after the task folder is final. The signatures cover
the task folder, so any change to `.env`, `addresses.json`, scripts, docs, or
validation files after this step requires regenerating them.

## Task Creator

```bash
cd mainnet/2026-06-01-update-sp1-gateway
make deps
