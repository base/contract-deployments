...
/// @notice Resets the AnchorStateRegistry starting anchor and updates the live multiproof
/// implementation in the DisputeGameFactory to the newly deployed AggregateVerifier carrying
...
contract UpdateVerifierHashes is MultisigScript {
    ...
    function _buildCalls() internal view override returns (Call[] memory) {
        ...
        // Call 0: ASR upgrade + re-init.
        //   - Reseeds the starting anchor root to (startingAnchorRootEnv,
        //     startingAnchorL2BlockNumberEnv).
        //   - Clears `anchorGame` to address(0) via the asr-reset-anchor-game.patch
        //     applied in `make deps`, so the new AggregateVerifier starts from a
        //     pristine anchor rather than the stale game pointer.
        //   - Bumps the proxy implementation to the freshly deployed
        //     AnchorStateRegistry impl whose ReinitializableBase number equals
        //     currentInitVersion + 1.
        calls[0] = Call({
        ...
        // Call 1: register the new AggregateVerifier impl in the DGF.
        calls[1] = Call({
            operation: Enum.Operation.Call,
    ...
    function _postCheck(Vm.AccountAccess[] memory, Simulation.Payload memory) internal override {
        ...
        // Verify all other AggregateVerifier immutables are preserved.
        require(
            address(nextAggregate.anchorStateRegistry()) == anchorStateRegistryProxyEnv, "next aggregate asr mismatch"
        );
        require(
