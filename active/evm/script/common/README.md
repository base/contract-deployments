...
| --- | --- | --- | --- |
| `verifier-update/` | `DeployAggregateVerifier.s.sol` | Deploys a replacement `AggregateVerifier` by copying immutable constructor inputs from the live implementation and replacing verifier hashes. | `config/<network>/.env`, `config/<network>/network.env`, `addresses.json` output |
| `verifier-update/` | `UpdateVerifierHashes.s.sol` | Multisig script that updates `DisputeGameFactory.gameImpls(gameType)` to a deployed `AggregateVerifier`. | `addresses.json` with `aggregateVerifier` |
...
Use exact Solidity pragmas based on the contracts the script imports; preserve a script's original exact pragma when moving it here. Load config from environment variables into immutable variables where possible. Keep task-specific values in `config/<network>/.env`, `config/<network>/network.env`, or task-local JSON files rather than hardcoding them in common scripts.
