...
HARDFORK_DIR = $(network)/$(shell date +'%Y-%m-%d')-upgrade-fault-proofs
...
TEMPLATE_HARDFORK = setup-templates/template-hardfork
...
# Run `make setup-hardfork network=<network>`
setup-hardfork:
	cp -r $(TEMPLATE_HARDFORK) $(HARDFORK_DIR)
